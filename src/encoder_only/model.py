...
└─────────────────┘
         ...
         ▼                       │
┌─────────────────┐              │
│ Pos Embedding   │  [seq_len] → [seq_len, d_model]
└─────────────────┘              │
         │                       │
         + ←─────────────────────┘ 相加
         │
...
│  │    ▼                                                        │  │
│  │  ┌─────────────┐                                            │  │
│  │  │ LayerNorm 1 │  ← Post-Norm: 先运算，后归一化              │  │
│  │  └─────────────┘                                            │  │
│  │    │                                                        │  │
│  │    ├────────────────────────────────────────┐               │  │
│  │    │                                        │               │  │
...
└─────────────────────────────────────────────────────────────────────┘
         ...
         ▼
┌─────────────────┐
│ out_proj        │  [batch, seq_len, d_model] → [batch, seq_len, vocab_size]
└─────────────────┘
         │
         ▼
输出 logits → softmax → argmax → 预测序列 [1, 1, 3, 4, 5]
